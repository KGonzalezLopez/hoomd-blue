    ...
    public:
        ...
        */
        DEVICE EvaluatorPairHertzian(Scalar _rsq, Scalar _rcutsq, const param_type& _params)
            : rsq(_rsq), rcutsq(_rcutsq), sigma(_params.x)
            {
            }

        //! Hertzian doesn't use diameter
        ...
        */
        DEVICE bool evalForceAndEnergy(Scalar& force_divr, Scalar& pair_eng, bool energy_shift)
            ...
            if (rsq < rcutsq) {
							Scalar r = sqrt(rsq);
							Scalar diff =  sigma - r;
							Scalar diff_three_halves = diff * sqrt(diff);

							pair_eng = (2.0/5.0) * diff_three_halves * diff;
							force_divr = (1.0/r) * diff_three_halves;
