...
#ifndef __PAIR_EVALUATOR_IPL_EDAN_H__
#define __PAIR_EVALUATOR_IPL_EDAN_H__
...
/*! \file EvaluatorPairIPLEdan.h
    \brief Defines the pair evaluator class for the IPL Edan potential. (See J. Chem. Phys. 148, 214502 (2018))
*/
...
struct ipl_edan_params
    {
    Scalar2 coeffs; //!< Contains epsilon, sigma2
    Scalar4 smoothing_constants; //!< c0, c2, c4, c6 to smooth potential.
};

class EvaluatorPairIPLEdan
    {
    public:
        //! Define the parameter type used by this pair potential evaluator
        typedef ipl_edan_params param_type;
        ...
        */
        DEVICE EvaluatorPairIPLEdan(Scalar _rsq, Scalar _rcutsq, const param_type& _params)
            : rsq(_rsq), rcutsq(_rcutsq), epsilon(_params.coeffs.x), one_over_sigma2(_params.coeffs.y),
            ...
            if (rsq < rcutsq)
                {
								...
								Scalar sigma12_over_r12 = Scalar(1.0) / (r2_over_sigma2*r2_over_sigma2*r2_over_sigma2*r2_over_sigma2*r2_over_sigma2*r2_over_sigma2);

								pair_eng = r2_over_sigma2*(sigma12_over_r12 + c2 + r2_over_sigma2*(c4 + r2_over_sigma2*c6)) + c0;
								force_divr = (Scalar(10)*sigma12_over_r12 - r2_over_sigma2*(Scalar(4)*c4 + r2_over_sigma2*Scalar(6)*c6) - Scalar(2)*c2)*one_over_sigma2;
								return true;
						}
            else
                return false;
...
#endif // __PAIR_EVALUATOR_IPL_EDAN_H__
